...
import { exifScrubber } from "@/lib/api/services/exif-scrubber.service";

export default function ExifScrubberTools() {
  ...
  const handleScrubber = async () => {
    ...
    try {
      ...
      const responsePromise = exifScrubber(
        file,
      ...
      if (err?.response?.data instanceof Blob) {
        try {
          ...
          setError(json.detail || "Metadata removal failed.");
        } catch {
          setError("Metadata removal failed. Please try again.");
        }
      } else {
        setError(
          err?.response?.data?.detail ||
            err?.message ||
            "Metadata removal failed. Please try again.",
        );
      }
    } finally {
      setTimeout(() => {
  ...
  return (
    <section className="max-w-3xl mx-auto bg-gradient-to-br from-white to-[#f9f6ff] border border-[#e9e1ff] rounded-[2rem] p-8 md:p-12 shadow-[0_20px_60px_rgba(91,50,180,0.15)] space-y-8">
        ...
        {!preview ? (
          <div className="space-y-4">
            ...
            <p className="text-lg font-bold text-[#2b1d3a]">
              Upload an image to remove metadata
            </p>
            <p className="text-sm text-gray-400">
              EXIF data will be permanently removed
            </p>
          </div>
        ) : (
          <div className="space-y-4">
      ...
      >
        {isGlobalLoading ? "Removing Metadata..." : "Remove EXIF Data"}
      </button>
      ...
      {optimizedBlob && !isGlobalLoading && (
        <div className="flex flex-col gap-3">
          <button
            onClick={() =>
              downloadBlob(optimizedBlob, `scrubbed-image-${Date.now()}`)
            }
