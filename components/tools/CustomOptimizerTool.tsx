...
import { optimizeCustomImage } from "@/lib/api/services/image-optimizer.service";
import { downloadBlob } from "@/lib/utils/download";
import { AxiosProgressEvent } from "axios";

export default function CustomOptimizerTool() {
    ...
    <section className="max-w-3xl mx-auto bg-gradient-to-br from-white to-[#f9f6ff] border border-[#e9e1ff] rounded-[2rem] p-8 md:p-12 shadow-[0_20px_60px_rgba(91,50,180,0.15)] space-y-8">
        ...
        {!preview ? (
          <div className="space-y-4">
            ...
            <p className="text-lg font-bold text-[#2b1d3a]">
              Drop your image for Custom Optimization
            </p>
            <p className="text-sm text-gray-400">
              Control quality, size and resize percentage
            </p>
          </div>
        ) : (
          <div className="space-y-4">
      ...
      {optimizedBlob && !isGlobalLoading && (
        <div className="flex flex-col gap-3">
          <button
            onClick={() => downloadBlob(optimizedBlob, `custom-${Date.now()}`)}
