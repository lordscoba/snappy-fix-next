...
import { optimizeWhatsAppImage } from "@/lib/api/services/image-optimizer.service";
import { downloadBlob } from "@/lib/utils/download";
import { AxiosProgressEvent } from "axios";

export default function WhatsAppOptimizerTool() {
  ...
  const handleOptimize = async () => {
    ...
    try {
      ...
      const responsePromise = optimizeWhatsAppImage(
        file,
  ...
  return (
    <section className="max-w-3xl mx-auto bg-gradient-to-br from-white to-[#f9f6ff] border border-[#e9e1ff] rounded-[2rem] p-8 md:p-12 shadow-[0_20px_60px_rgba(91,50,180,0.15)] space-y-8">
      ...
      <button
        ...
        aria-label="optimize"
      >
        {isGlobalLoading ? "Optimizing..." : "Optimize for WhatsApp"}
      </button>
      ...
      {optimizedBlob && !isGlobalLoading && (
        <div className="flex flex-col gap-3">
          <button
            onClick={() =>
              downloadBlob(optimizedBlob, `whatsapp-${Date.now()}`)
            }
