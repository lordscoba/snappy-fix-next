...
import { optimizeSeoResponsiveImage } from "@/lib/api/services/image-optimizer.service";
import { downloadBlob } from "@/lib/utils/download";
import { AxiosProgressEvent } from "axios";

export default function SeoResponsiveOptimizerTool() {
  ...
  const handleOptimize = async () => {
    ...
    try {
      ...
      const responsePromise = optimizeSeoResponsiveImage(
        file,
  ...
  return (
    <section className="max-w-3xl mx-auto bg-gradient-to-br from-white to-[#f9f6ff] border border-[#e9e1ff] rounded-[2rem] p-8 md:p-12 shadow-[0_20px_60px_rgba(91,50,180,0.15)] space-y-8">
      ...
      >
        {isGlobalLoading ? "Optimizing..." : "Optimize for SEO & Responsive"}
      </button>
      ...
      {optimizedBlob && !isGlobalLoading && (
        <div className="flex flex-col gap-3">
          <button
            onClick={() =>
              downloadBlob(optimizedBlob, `seo-responsive-${Date.now()}`)
            }
