...
import { heicToImage } from "@/lib/api/services/heic-converter.service";
import { downloadBlob } from "@/lib/utils/download";
import { AxiosProgressEvent } from "axios";

export default function HeicToImageTools() {
  ...
  const handleFile = (selectedFile: File) => {
    if (!selectedFile.name.toLowerCase().endsWith(".heic")) {
      setError("Please upload a valid HEIC image.");
      return;
    }
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const responsePromise = heicToImage(
        file,
        { format },
        ...
        {!preview ? (
          <div className="space-y-4">
            ...
            <p className="text-lg font-bold text-[#2b1d3a]">
              Drop your HEIC image
            </p>
            <p className="text-sm text-gray-400">Convert HEIC to JPG or PNG</p>
          </div>
        ) : (
          <div className="space-y-4">
            {/* Because we converted HEIC to JPEG for the preview state, 
        this <img> tag will now work perfectly in Safari, Chrome, and Firefox.
      */}
            <img
              src={preview}
              alt="Preview"
              className="max-h-64 mx-auto rounded-2xl shadow-lg border-4 border-white transition-all duration-300"
            />

            <div className="flex flex-col items-center">
              <p className="text-sm font-medium text-gray-500 truncate max-w-[250px]">
                {file?.name}
              </p>
              {file?.name.toLowerCase().endsWith(".heic") && (
                <span className="text-[10px] font-bold text-indigo-500 uppercase tracking-tighter bg-indigo-50 px-2 py-0.5 rounded-md mt-1">
                  HEIC Optimized Preview
                </span>
              )}
            </div>
          </div>
      ...
      {optimizedBlob && !isGlobalLoading && (
        <div className="flex flex-col gap-3">
          <button
            onClick={() =>
              downloadBlob(optimizedBlob, `converted-${Date.now()}.${format}`)
            }
