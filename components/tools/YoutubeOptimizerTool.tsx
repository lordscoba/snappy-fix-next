...
import { optimizeYouTubeThumbnail } from "@/lib/api/services/image-optimizer.service";
...
export default function InstagramOptimizerTool() {
  ...
  const handleOptimize = async () => {
    ...
    try {
      ...
      const responsePromise = optimizeYouTubeThumbnail(
        file,
  ...
  return (
    <section className="max-w-3xl mx-auto bg-gradient-to-br from-white to-[#f9f6ff] border border-[#e9e1ff] rounded-[2rem] p-8 md:p-12 shadow-[0_20px_60px_rgba(91,50,180,0.15)] space-y-8">
      ...
      >
        {isGlobalLoading ? "Optimizing..." : "Optimize for YouTube Thumbnails"}
      </button>
      ...
      {optimizedBlob && !isGlobalLoading && (
        <div className="flex flex-col gap-3">
          <button
            onClick={() => downloadBlob(optimizedBlob, `youtube-${Date.now()}`)}
